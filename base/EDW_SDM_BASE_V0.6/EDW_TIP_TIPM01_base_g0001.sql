...
CAST(ROWID AS VARCHAR2(100)) AS CHK_ID,
'num_err' AS DATA_CHK_TP,
'OAMT' AS DATA_CHK_COL,
OAMT AS DATA_VAL,
SDG.UFN_NUMCHECK(OAMT) AS CHECK_RESULT
FROM ${SOURCE_TABLE_OWNER}.${SOURCE_TABLE} 
WHERE DATA_EXCH_DATE = TO_DATE('${TXDATE}', 'YYYY-MM-DD')
) S1
WHERE S1.CHECK_RESULT = '0';
...
WSNO,
TXTNO,
SDG.UFN_NORM_DATE(CASE WHEN NVL(TRIM(ACYYY), '')='' THEN NULL WHEN SDG.UFN_ADDATECHECK_DATE(TRIM(ACYYY))='0' THEN NULL ELSE ACYYY END) AS ACYYY,
CODE,
NO,
IO,
ACTCD,
...
ECCD,
OAMT,
SDG.UFN_NORM_DATE(CASE WHEN NVL(TRIM(ETXDT), '')='' THEN NULL WHEN SDG.UFN_ADDATECHECK_DATE(TRIM(ETXDT))='0' THEN NULL ELSE ETXDT END) AS ETXDT,
SDG.UFN_NORM_DATE(CASE WHEN NVL(TRIM(ECINDT), '')='' THEN NULL WHEN SDG.UFN_ADDATECHECK_DATE(TRIM(ECINDT))='0' THEN NULL ELSE ECINDT END) AS ECINDT,
ECWSNO,
ECTXTNO,
SDG.UFN_NORM_DATE(CASE WHEN NVL(TRIM(CKDT), '')='' THEN NULL WHEN SDG.UFN_ADDATECHECK_DATE(TRIM(CKDT))='0' THEN NULL ELSE CKDT END) AS CKDT,
CURRENT_TIMESTAMP,
