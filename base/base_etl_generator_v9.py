...
from rule_config_v4 import * ##20250516 - 改為rule_config_v4
from sql_template_v2 import *
import time
import sys
from openpyxl import load_workbook

##[20250514][蔡雯欣][修改rule_config/version/constants/filter_index_rows修改constants/filter_index_rows]
...
def set_version():
    ...
    TARGET_SHEET = f'Base資料欄位對應{VER}'
    OUTPUT_DIR = f'EDW_SDM_BASE_{VERSION}' ##20250514 - 改為SDM名稱+版號
...
def generate_sql_for_table(args):
    ...
    # Write to file
    with open(os.path.join(OUTPUT_DIR, f'{table_name}_base_g0001.sql'), 'w', encoding='utf-8') as f:
...
def process_transform_rows(data_row, audit_tables, OUTPUT_DIR):
    ...
    for table_name, group in grouped_rows:
        ...
        with open(os.path.join(OUTPUT_DIR, f'{table_name}_base_g0001.sql'), 'a', encoding='utf-8') as f:
            f.write(file_content + '\n')
        ...
        if file_counter is not None and file_counter_lock is not None:
            with file_counter_lock:
                ...
                print(f"Generated {file_counter.value} files: {table_name}_base_g0001.sql")

        else:
            print(f"Generated file: {table_name}_base_g0001.sql")
